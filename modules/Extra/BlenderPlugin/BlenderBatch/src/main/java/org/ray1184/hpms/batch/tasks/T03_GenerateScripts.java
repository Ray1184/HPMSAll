    ...
    public enum ScriptSections {
        ...
        CLEANUP_POST {
            ...
            public LuaStatement getCustomStatement(HPMSParams params, SceneDataResponse.RoomInfo roomInfo) {
                ...
                SceneObject.filter(roomInfo, SceneObject.VIEW_ACTIVATOR).forEach(a -> SceneObject.filter(roomInfo, SceneObject.CAMS).stream().filter(c -> a.getEvents().stream().anyMatch(e -> e.getParams().contains(c.getName()))).forEach(c -> {

    ...
    public void handleSetupPre(LuaStatementBuilder builder, SceneDataResponse.RoomInfo roomInfo) {
        SceneObject.filter(roomInfo, super.sceneObject).forEach(a -> SceneObject.filter(roomInfo, SceneObject.CAMERA).stream()//
                ...
                .forEach(c -> {
                    ...
                    if (SceneObject.filter(roomInfo, SceneObject.PERIMETER).stream().findFirst().isPresent()) {
                        builder.expr("sceneManager:sample_view_by_callback(function() if currentSector ~= nil then return currentSector.id == '?' else return false end end, background_?, lib.vec3(?, ?, ?), lib.quat(?, ?, ?, ?))",

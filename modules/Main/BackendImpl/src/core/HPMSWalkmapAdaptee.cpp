...
void hpms::WalkmapAdaptee::ForEachPathStep(const std::function<bool(PathStepAdapter*)>& visitor)
{
	Check(walkmap.get());
	for (const auto& path : walkmap->GetData()->GetPaths())
	{
		bool exit = visitor(paths[path]);
		if (exit)
		{
			return;
		}
...
bool hpms::PathStepAdaptee::IsBound(PathStepAdapter* path)
{
	auto* adaptee = static_cast<PathStepAdaptee*>(path);

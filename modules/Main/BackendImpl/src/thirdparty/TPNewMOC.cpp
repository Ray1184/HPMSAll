        ...
        for (auto query_result = results.begin(); query_result != results.end(); ++query_result)
        {
            ...
            // only check this result if its a hit against an entity
            {
                ...
                for (size_t i = 0; i < data.index_count; i += 3)
                {
                    ...
                    {
                        // FORCED_FIX
                        mat = reinterpret_cast<const Ogre::Matrix4*>(&query_result->first->Entity->getParentNode()->_getFullTransform());
                    }
        ...
        for (auto data = m_Entities.begin(); (data != m_Entities.end() && !Stop); ++data)
        {
            ...
            switch (data->CollisionType)
            {
                ...
                case COLLISION_BOX:
                {
                    ...
                    {
                        // FORCED_FIX
                        ret.push_back(CollisionTools::RayQueryEntry(&data[0], inter.second));
